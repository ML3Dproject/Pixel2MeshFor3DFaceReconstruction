...
class Evaluator(CheckpointRunner):
    ...
    def evaluate_nme_unormal(self,pred_vertices, gt_points, labels):
        batch_size = pred_vertices.size(0)
        for i in range(batch_size):
            gt_length = gt_points[i].size(0)
            label = labels[i].cpu().item()
            d1, d2, i1, i2 = self.chamfer(pred_vertices[i].unsqueeze(0), gt_points[i].unsqueeze(0))
            d1, d2 = d1.cpu().numpy(), d2.cpu().numpy()  # convert to millimeter
            return self.nme_unormal[label].update(self.normalized_mean_error(d1, d2, 1000., gt_length))
        
            
    def evaluate_nme(self,pred_vertices, gt_points, labels, img,lc,nme):
        batch_size = pred_vertices.size(0)
        for i in range(batch_size):
            gt_length = gt_points[i].size(0)
            label = labels[i].cpu().item()
            ...
            d1, d2 = d1.cpu().numpy(), d2.cpu().numpy()  # convert to millimeter
            if nme[i] != 0:
                return self.nme[label].update(self.normalized_mean_error(d1, d2, nme[i], gt_length))
            else:
                return self.nme[label]
    def normalized_mean_error(self, dis_to_pred, dis_to_gt, face_size, gt_length):
        error = 0.5 * (np.mean(dis_to_pred) + np.mean(dis_to_gt))
        nme = error / (gt_length * face_size/1000.)
        return nme
    ...
    def evaluate_step(self, input_batch):
        ...
        with torch.no_grad():
            ...
            # detectimg = input_batch['img_copy']
            ...
            if self.options.model.name == "pixel2mesh":
                ...
                self.evaluate_nme(pred_vertices, gt_points, input_batch["labels"],images,lc,nme)
                self.evaluate_nme_unormal(pred_vertices, gt_points, input_batch["labels"])
            elif self.options.model.name == "classifier":
    ...
    def evaluate(self):
        ...
        if self.options.model.name == "pixel2mesh":
            ...
            self.nme_unormal = [AverageMeter() for _ in range(self.num_classes)]
        elif self.options.model.name == "classifier":
    ...
    def evaluate_summaries(self, input_batch, out_summary):
        self.logger.info("Test Step %06d/%06d (%06d) " % (self.evaluate_step_count,
                         ...
                         + ", ".join([key + "_cur:" + "%.6f" % val[0].val + " "+ "_avg:"+ "%.6f" % val[0].avg
                                      for key, val in self.get_result_summary().items()]))
